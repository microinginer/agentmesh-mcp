...
export function BlockerRadar({ pulse }: { pulse: DailyPulseResponse }) {
  ...
  return (
    <Alert variant="destructive" className="border-red-500/30 bg-red-500/10 text-red-950 dark:text-red-200">
      ...
      <AlertDescription className="mt-2 space-y-3">
        {blockedAgents.map((b, i) => (
          <div
            key={i}
            className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 p-2.5 rounded-md bg-background/60 border border-red-500/20 text-foreground text-xs"
          >
            <div className="space-y-1">
              <div className="flex items-center gap-2 flex-wrap">
                ...
                <span className="text-muted-foreground">• {b.developerName} ({b.connectionLabel})</span>
                <Badge variant="destructive" className="text-[10px] h-4 py-0">Blocked</Badge>
              </div>
              <p className="font-mono text-xs text-red-900 dark:text-red-300 font-medium">
            ...
            {b.filesTouched.length > 0 && (
              <div className="flex items-center gap-1 flex-wrap text-[11px] text-muted-foreground">
                <span className="font-mono">{b.filesTouched.slice(0, 2).join(", ")}</span>
